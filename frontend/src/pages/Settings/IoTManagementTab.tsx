...
type TabType = "tuya" | "ezviz";

export default function IoTManagementTab() {
  const [activeTab, setActiveTab] = useState<TabType>("tuya");
  ...
  // ── EZVIZ States ──
  const [ezvizUsername, setEzvizUsername] = useState("");
  const [ezvizPassword, setEzvizPassword] = useState("");
  const [ezvizRegion, setEzvizRegion] = useState("apiisgp.ezvizlife.com");
  const [ezvizShowPass, setEzvizShowPass] = useState(false);
  const [ezvizTesting, setEzvizTesting] = useState(false);
  const [ezvizTestResult, setEzvizTestResult] = useState<{
    success: boolean;
    message: string;
    cameras?: any[];
  } | null>(null);
  const [ezvizSaving, setEzvizSaving] = useState(false);
  ...
  const handleEzvizTest = async () => {
    if (!ezvizUsername || !ezvizPassword) {
      setEzvizTestResult({
        success: false,
    ...
    setEzvizTesting(true);
    setEzvizTestResult(null);
    try {
      const res = await iotService.testEzvizConnection({
        username: ezvizUsername,
        password: ezvizPassword,
        region: ezvizRegion,
      });
      setEzvizTestResult(res);
    } catch (err: any) {
      setEzvizTestResult({
        success: false,
        message: err?.response?.data?.detail || "Lỗi kết nối EZVIZ Cloud.",
      });
    } finally {
      setEzvizTesting(false);
    }
  };

  const handleEzvizSave = async (e: FormEvent) => {
    e.preventDefault();
    if (!ezvizTestResult?.success) return;
    setEzvizSaving(true);
    try {
      await iotService.createDevice({
        name: `EZVIZ Account (${ezvizUsername})`,
        provider: "ezviz",
        device_type: "camera",
        config_data: {
          username: ezvizUsername,
      ...
      setEzvizTestResult({
        success: true,
        message: "Đã lưu cấu hình EZVIZ thành công!",
      });
      setEzvizUsername("");
      setEzvizPassword("");
      fetchDevices();
      ...
      setEzvizTestResult({
        success: false,
        message: err?.response?.data?.detail || "Lỗi lưu cấu hình EZVIZ.",
      });
    } finally {
      setEzvizSaving(false);
    }
  };

  // Filtered devices by tab
  const tuyaDevices = devices.filter((d) => (d as any).provider !== "ezviz");
  const ezvizDevices = devices.filter((d) => (d as any).provider === "ezviz");
      ...
      >
        <button
          ...
          style={{
            ...
            color:
              activeTab === "tuya" ? "var(--primary)" : "var(--text-muted)",
            fontWeight: activeTab === "tuya" ? 600 : 400,
            cursor: "pointer",
            display: "flex",
            alignItems: "center",
            gap: "8px",
        ...
        >
          <Wifi size={16} /> Ổ cắm thông minh (Tuya)
        </button>
        <button
          onClick={() => setActiveTab("ezviz")}
          style={{
            padding: "10px 20px",
            background: "transparent",
            border: "none",
            borderBottom:
              activeTab === "ezviz"
                ? "2px solid #10b981"
                : "2px solid transparent",
            color: activeTab === "ezviz" ? "#10b981" : "var(--text-muted)",
            fontWeight: activeTab === "ezviz" ? 600 : 400,
            cursor: "pointer",
            display: "flex",
            alignItems: "center",
            gap: "8px",
        ...
        >
          <Camera size={16} /> Camera an ninh (EZVIZ)
        </button>
      </div>
      ...
      {activeTab === "tuya" && (
        <>
          ...
          >
            ...
            >
              <Info size={16} /> Hướng dẫn cấu hình
            </button>
            <button
          ...
          {showGuide && (
            ...
            >
              ...
              >
                <li>
                  Tải app SmartLife/Tuya, kết nối các ổ cắm vào cùng chung mạng
                  WiFi với máy chủ này.
                </li>
                <li>
                  ...
                  , tạo 1 Cloud Project và liên kết App SmartLife vào Project
                  đó.
                </li>
                <li>
                  ...
                  <strong>Access Secret/Client Secret</strong> (vào ô API
                  Secret) khi được hỏi.
                </li>
                <li>
                  ...
                  . Mở file này ra để lấy chính xác <strong>Device ID</strong>{" "}
                  và <strong>Local Key</strong> của từng thiết bị.
                </li>
              </ol>
              <div
                style={{
              ...
              >
                ...
                <strong>Thêm thiết bị mới -&gt; Quét Radar Tự Động</strong> để
                quét IP cục bộ nhanh nhất.
              </div>
            </div>
          )}
              ...
              <div className={styles.modalContent}>
                ...
                {scannedDevices !== null && (
                  ...
                  >
                    ...
                    >
                      <Radar size={16} /> Radar tìm thấy (
                      {scannedDevices.length}) thiết bị qua sóng WiFi LAN:
                    </div>
                    {scannedDevices.length === 0 ? (
                      <div
                        style={{ color: "var(--text-muted)", fontSize: "13px" }}
                      >
                        Không tìm thấy ổ cắm Tuya nào đang cắm điện cùng mạng
                        WiFi với Server.
                      </div>
                    ) : (
                  ...
                  {newDevice.device_type === "multi" && (
                    ...
                    >
                      ...
                      Object.keys(newDevice.dps_mapping).length === 0 ? (
                        ...
                        >
                          Chưa có cổng nào. Hãy ấn Test Kết Nối để tự động lấy,
                          hoặc tự Thêm Cổng.
                        </div>
                      ) : (
          ...
          >
            ...
            {tuyaDevices.length === 0 && !isAdding && (
              ...
              >
                Không có ổ cắm Tuya nào được quản lý. Ấn Thêm Thiết Bị để bắt
                đầu.
              </div>
            )}
          </div>
          ...
          {/* Pagination Controls */}
          {tuyaDevices.length > ITEMS_PER_PAGE && (
            <div
              style={{
                display: "flex",
            ...
            >
              ...
              <span style={{ fontSize: "14px", color: "var(--text-muted)" }}>
                Trang {currentPage} /{" "}
                {Math.ceil(tuyaDevices.length / ITEMS_PER_PAGE)}
              </span>
              <button
                onClick={() =>
                  setCurrentPage((p) =>
                ...
                disabled={
                  currentPage === Math.ceil(tuyaDevices.length / ITEMS_PER_PAGE)
                }
                style={{
                  background: "var(--bg-secondary)",
                  border: "1px solid var(--border-light)",
        ...
        <div>
          ...
          {ezvizDevices.length > 0 && (
            <div style={{ marginBottom: "20px" }}>
              ...
              >
                Tài khoản EZVIZ đã cấu hình:
              </div>
              <div style={{ display: "grid", gap: "12px" }}>
                {ezvizDevices.map((dev) => (
                  <div
                    key={dev.id}
                    className={styles.card}
                  ...
                  >
                    ...
                    >
                      ...
                      <div>
                        ...
                        >
                          Tài khoản:{" "}
                          {(dev as any).config_data?.username || "N/A"} • Vùng:{" "}
                          {(dev as any).config_data?.region || "Auto"}
                        </span>
                      </div>
                    </div>
                  </div>
          ...
          <div className={styles.card}>
            ...
            >
              <Camera size={18} color="#10b981" /> Kết nối tài khoản EZVIZ
            </h3>
            <p
              style={{
                color: "var(--text-muted)",
            ...
            >
              Nhập tài khoản EZVIZ (dùng trên App) để JARVIS có thể truy cập
              Camera của bạn. Lưu ý: Số điện thoại nhập theo định dạng{" "}
              <b>84xxxxxxxxx</b> (không có dấu + hoặc số 0 đầu).
            </p>

            {ezvizTestResult && (
              <div
                className={
                  ezvizTestResult.success ? styles.success : styles.error
                }
                style={{ marginBottom: "16px" }}
              >
                {ezvizTestResult.success
                  ? `✅ ${ezvizTestResult.message}`
                  : `❌ ${ezvizTestResult.message}`}
              </div>
            )}

            <form onSubmit={handleEzvizSave}>
              <div
                style={{
                  display: "grid",
                  gridTemplateColumns: "1fr 1fr",
              ...
              >
                <div className={styles.field}>
                  ...
                  <input
                    placeholder="84387839744"
                    value={ezvizUsername}
                    onChange={(e) => setEzvizUsername(e.target.value)}
                    required
                  />
                </div>
                <div className={styles.field} style={{ position: "relative" }}>
                  <label>Mật khẩu EZVIZ</label>
                  <input
                    type={ezvizShowPass ? "text" : "password"}
                    placeholder="••••••••"
                    value={ezvizPassword}
                  ...
                  <button
                    type="button"
                    onClick={() => setEzvizShowPass(!ezvizShowPass)}
                    style={{
                      position: "absolute",
                      right: "8px",
                      bottom: "8px",
                  ...
                  >
                    {ezvizShowPass ? <EyeOff size={16} /> : <Eye size={16} />}
                  </button>
                </div>
                <div className={styles.field}>
                  <label>Vùng máy chủ (Region)</label>
                  <select
                    value={ezvizRegion}
                    onChange={(e) => setEzvizRegion(e.target.value)}
                  >
                    <option value="apiisgp.ezvizlife.com">
                      Singapore / Việt Nam (SEA)
                    </option>
                    <option value="apiieu.ezvizlife.com">Châu Âu (EU)</option>
                    <option value="api.ezvizlife.com">Global</option>
                    <option value="apiius.ezvizlife.com">Mỹ (US)</option>
                  </select>
                </div>
              </div>
              ...
              >
                <button
                  ...
                  onClick={handleEzvizTest}
                  disabled={ezvizTesting}
                >
                  {ezvizTesting
                    ? "Đang kết nối Cloud..."
                    : "Test Kết Nối EZVIZ"}
                ...
                <button
                  ...
                  disabled={!ezvizTestResult?.success || ezvizSaving}
                  title={
                    !ezvizTestResult?.success
                      ? "Chỉ được lưu sau khi Test kết nối thành công"
                      : ""
                  }
                >
                  {ezvizSaving ? "Đang lưu..." : "Lưu Cấu Hình"}
                </button>
              </div>
              ...
              {!ezvizTestResult?.success && (
                ...
                >
                  *Bắt buộc phải Test kết nối Cloud thành công trước khi Lưu
                </div>
              )}
            </form>
          </div>
